...
public class IntervalFitterTests
{
    ...
    public async Task PrextValidDomain(string dataSetName, string campType, int numIntervalsExpected)
    {
        ...
        intervals = await IntervalFitter.FitIntervals(intervals, colorMap, timeoutInSecs: 15);
    ...
    [Fact]
    public async Task PrextValidDomainEmptyList()
    {
        (List<Interval>? intervals, Dictionary<int, int> colorMap) = (new List<Interval>(), new Dictionary<int, int>());
        intervals = await IntervalFitter.FitIntervals(intervals, colorMap, timeoutInSecs: 15);
    ...
    public async Task PrextInValidDomainOverlappingIntervals(string dataSetName, string campType)
    {
        ...
        await Assert.ThrowsAsync<ArgumentException>( async () => await IntervalFitter.FitIntervals(intervals, colorMap, timeoutInSecs: 15));
    }
